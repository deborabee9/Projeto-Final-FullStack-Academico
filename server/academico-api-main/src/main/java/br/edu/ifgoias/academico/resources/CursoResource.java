...
public class CursoResource {
    ...
    public ResponseEntity<Page<CursoDTO>> listaCursos( 	@RequestParam(defaultValue = "0") int page,
    													@RequestParam(defaultValue = "10") int size,
    													@RequestParam(required = false) String search) {
        Page<CursoDTO> cursos;
        if (search != null && !search.trim().isEmpty()) {
            cursos = servico.listaCurso(page, size, search);
        } else {
            cursos = servico.listaCurso(page, size);
        }
    ...
    public ResponseEntity<String> cursoDeletar(@PathVariable Integer id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.OK).body("Curso id:" + id + " excluido!" );
        } catch (RuntimeException ex) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, ex.getMessage());
        }
    }
