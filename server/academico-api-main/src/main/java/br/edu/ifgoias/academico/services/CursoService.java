...
public class CursoService {
    ...
    @Transactional
    public Page<CursoDTO> listaCurso(int page, int size) {
        PageRequest pageable = PageRequest.of(page, size);
        return cursoRep.findAll(pageable)
    ...
    public Page<CursoDTO> listaCurso(int page, int size, String search) {
        ...
        if (search == null || search.trim().isEmpty()) {
            ...
            return cursoRep.findByNomeCursoContainingIgnoreCase(search.trim(), pageable)
                          .map(curso -> cursoMapper.toDTO(curso));
        }
    }
